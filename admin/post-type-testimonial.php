...
function themify_theme_testimonial_meta_box( $args = array() ){
	...
	return array(
		...
		array(
			...
			'description' => __('To show this use [testimonial id="%s"]'),
			'type' 		=> 'post_id_info'
		)
	);
}
			...
			if ($posts) {
				...
				$out .= '<div '.$slider_id.' class="loops-wrapper shortcode ' . $post_type  . ' ' . $style . ' '. $cpt_layout_class .'">';
					if ( 'slider' == $style ) {
						...
						$out .= sprintf('<div class="slideshow-wrap"><ul class="slideshow testimonial-holder" data-id="testimonial-slider-%s" data-autoplay="%s" data-effect="%s" data-speed="%s" data-wrap="%s" data-slidernav="yes" data-pager="yes">',
							$this->instance,
